...
export function useOptimizedProducts() {
  ...
  const loadProducts = useCallback(async (forceRefresh = false) => {
    ...
    try {
      ...
      if (validProducts.length !== productList.length) {
        console.warn(`${productList.length - validProducts.length} produtos inválidos foram filtrados`);
      }
  ...
  const getProductsByUnit = useCallback((unit: Unit): Product[] => {
    return productsByUnit.get(unit) || [];
  }, [productsByUnit]);

  // Filtrar produtos por nome
  const filterProducts = useCallback((searchTerm: string): Product[] => {
