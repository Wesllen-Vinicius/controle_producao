...
export const fetchBalances = async (products: Product[]): Promise<Balance[]> => {
...
export const fetchTransactionsPage = async (
  ...
  return { page: page as Transaction[], hasMore: page.length === PAGE_SIZE };
};
...
export const addTransaction = async (payload: AddTransactionPayload): Promise<void> => {
  const { error } = await supabase.from('inventory_transactions').insert(payload);
  if (error) {
    if (error.message.includes('duplicate')) throw new Error('Transação duplicada detectada.');
