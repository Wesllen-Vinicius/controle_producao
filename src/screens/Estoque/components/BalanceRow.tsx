...
const BalanceRow = memo(function BalanceRow({
  ...
  const deltaText = hasDelta
    ? `${up ? '↗' : '↘'} ${formatQuantity(unit, Math.abs(todayDelta ?? 0))}`
    : '';

  const statusColor = neg ? colors.danger : low ? '#FF8C00' : colors.success;
  const statusIcon: React.ComponentProps<typeof MaterialCommunityIcons>['name'] = neg
    ? 'alert-circle'
    : low
      ? 'alert'
      : 'check-circle';
      ...
      <Animated.View style={{ opacity: fadeAnim }}>
        ...
        >
          ...
          >
            <View>
              <Text
                style={{ fontWeight: '900', fontSize: 28, color: statusColor, letterSpacing: -0.5 }}
              >
                {qtyStr}
              </Text>
              {neg && (
                <Text
                  style={{ color: colors.danger, fontSize: 11, fontWeight: '700', marginTop: -2 }}
                >
                  ESTOQUE NEGATIVO
                </Text>
              )}
              {low && !neg && (
                <Text style={{ color: '#FF8C00', fontSize: 11, fontWeight: '700', marginTop: -2 }}>
                  ESTOQUE BAIXO
                </Text>
              )}
            </View>
