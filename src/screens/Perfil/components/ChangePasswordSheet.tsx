...
export function ChangePasswordSheet({ open, onClose, email }: ChangePasswordSheetProps) {
  ...
  const handleChangePassword = useCallback(async () => {
    ...
    try {
      ...
      h.error();
      const msg = e.message === 'Senha atual incorreta.' ? e.message : 'Falha ao alterar a senha. Tente novamente.';
  ...
  return (
    <BottomSheet open={open} onClose={handleClose} title="Alterar Senha">
      <View style={{ gap: spacing.md, padding: spacing.md }}>
        <Input label="Senha atual" value={currentPassword} onChangeText={setCurrentPassword} secureTextEntry autoComplete="current-password" />
        <Input label="Nova senha" value={newPassword} onChangeText={setNewPassword} secureTextEntry autoComplete="new-password" />
        <Input label="Confirmar nova senha" value={confirmPassword} onChangeText={setConfirmPassword} secureTextEntry autoComplete="new-password" />
        <View style={{ marginTop: spacing.md, gap: spacing.sm }}>
          <Button title="Salvar Alterações" onPress={handleChangePassword} loading={loading} disabled={loading} />
