...
function getExtra<T = any>(key: string, fallback?: T): T | undefined {
  ...
  const easExtra = (Constants.expoConfig as any)?.extra?.eas ?? {};
...
export const APP_VERSION = (Constants.expoConfig as any)?.version ?? "—";
export const BUILD_NUMBER =
  (Constants.expoConfig as any)?.ios?.buildNumber?.toString() ??
