...
export const fetchHistory = async (filters: ProductionFilters): Promise<Production[]> => {
  let query = supabase
    .from('productions')
...
export const fetchSummaryItems = async (productionId: string): Promise<SummaryItem[]> => {
  const { data, error } = await supabase
...
type SavePayload = {
  ...
  prod_date: string;
  abate: number;
  items: { product_id: string; produced: number }[];
};
