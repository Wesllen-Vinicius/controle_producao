...
export default function ProducaoScreen() {
    ...
    const {
        loading, refreshing, onRefresh, historyItems, itemsCache, products, productionStats,
        formOpen, setFormOpen, prodDate, setProdDate, abateStr, setAbateStr,
        selectedIds, producedQuantities, setProducedQuantities, saving, abate,
        showFilters, setShowFilters, filters, setFilters,
        handleSave, loadItemDetails, toggleProduct, selectAll, clearSelection, prodNum
    } = useProductionData();
    ...
    const renderItem: ListRenderItem<Renderable> = useCallback(({ item }) => {
        if (item.type === 'h-header') {
            return (
                <View style={{ paddingHorizontal: spacing.lg, marginTop: spacing.xl, marginBottom: spacing.md, flexDirection: 'row', alignItems: 'center', gap: spacing.md }}>
                    <Text style={{ fontWeight: '700', color: colors.text, fontSize: 16 }}>{item.title}</Text>
    ...
    if (loading) {
        return <View style={{ paddingHorizontal: spacing.md, flex: 1 }}><SkeletonList rows={10} /></View>;
    }
        ...
        <Screen padded={false} scroll={false}>
            ...
            <BottomSheet open={formOpen} onClose={() => setFormOpen(false)} title="Registrar Produção">
                <ProductionForm
                    ...
                    products={products} selected={selectedIds}
                    toggleProduct={toggleProduct} selectAll={selectAll} clearSelection={clearSelection}
                />

                {selectedProducts.length > 0 && <View style={{ height: 1, backgroundColor: colors.line, marginVertical: spacing.lg, opacity: 0.5 }} />}
