...
import { RelatorioSkeleton } from '../../components/Skeletons';
...
export default function AdminProductionsReportScreen() {
  ...
  const {
    ...
    hasProductFilter,
    chartSeries,
    chartUnit, // 2. Obter dados para o gráfico do hook
    dateRange,
    setDateRange,
    productFilters,
    setProductFilters,
  ...
  const renderItem = useCallback<ListRenderItem<DayTotals>>(
    ({ item }) => (
  ...
  if (profile?.role !== 'admin') {
    return (
      <EmptyState
        title="Acesso Negado"
        subtitle="Esta área é restrita para administradores."
  ...
  const ListEmpty = () => {
    ...
    return (
      <EmptyState
        ...
        subtitle="Não há dados de produção para os filtros e o período selecionados."
      />
    );
  };
