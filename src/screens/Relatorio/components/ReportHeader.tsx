...
export const ReportHeader = React.memo((props: ReportHeaderProps) => {
  ...
  const { onExportPress, dateRange, onDateChange, onApplyFilters, products, productFilters, onProductFilterChange, unitFilter, onUnitFilterChange } = props;

  const quickRange = useCallback((days: 7 | 30 | 'month') => {
    const now = new Date();
  ...
  const toggleProductFilter = useCallback((id: string) => {
    const newFilters = productFilters.includes(id) ? productFilters.filter(pId => pId !== id) : [...productFilters, id];
    ...
    <View style={{ gap: spacing.md, marginBottom: spacing.md, paddingTop: spacing.sm }}>
      <View style={[styles.pageHeader, {paddingHorizontal: spacing.md}]}>
        <View>
          ...
          <Text style={[typography.body, { color: colors.muted }]}>Análise de performance da produção</Text>
        </View>
        <Pressable onPress={onExportPress} style={({pressed}) => [styles.exportButton, { backgroundColor: colors.primary, opacity: pressed ? 0.8 : 1 }]}>
      ...
      <Card style={{marginHorizontal: spacing.md}} padding="md">
        ...
        <View style={styles.dateFieldsContainer}>
            <DateField label="Início" value={dateRange.from} onChange={from => onDateChange({ ...dateRange, from })} />
            <DateField label="Fim" value={dateRange.to} onChange={to => onDateChange({ ...dateRange, to })} />
        </View>
        ...
        <Button title="Aplicar Período" onPress={onApplyFilters} style={{marginTop: spacing.md}}/>
      </Card>
      ...
      <Card style={{marginHorizontal: spacing.md}} padding="md">
        ...
        <ScrollView horizontal showsHorizontalScrollIndicator={false}>
            <View style={styles.chipContainer}>
                <Chip label="Todos" active={productFilters.length === 0} onPress={() => onProductFilterChange([])} />
                {products.map(p => (
                    <Chip key={p.id} label={p.name} active={productFilters.includes(p.id)} onPress={() => toggleProductFilter(p.id)} />
                ))}
            </View>
        </ScrollView>
        {productFilters.length > 0 && unitsAvailable.length > 1 && (
            <View style={{ marginTop: spacing.md}}>
                ...
                <ScrollView horizontal showsHorizontalScrollIndicator={false} style={{marginTop: spacing.sm}}>
                    <View style={styles.chipContainer}>
                        <Chip label="Todas" active={unitFilter === 'ALL'} onPress={() => onUnitFilterChange('ALL')} />
                        {unitsAvailable.map(u => (
                            <Chip key={u} label={u.toUpperCase()} active={unitFilter === u} onPress={() => onUnitFilterChange(u)} />
                        ))}
                    </View>
                </ScrollView>
            </View>
