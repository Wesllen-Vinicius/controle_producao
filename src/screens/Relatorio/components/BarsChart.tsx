  ...
  ({
    ...
    return (
      <View style={styles.barGroup}>
        ...
        <Animated.View
          style={[styles.bar, { height: metaAnim, backgroundColor: colorMeta, opacity: 0.6 }]}
        />
      </View>
    );
  }
...
export function BarsChart({
  ...
  const maxValue = useMemo(
    () => Math.max(1, ...slicedData.flatMap(d => [d.produced, d.meta])),
    [slicedData]
  );
    ...
    <View style={{ gap: spacing.sm }}>
      <View style={styles.legendContainer}>
        <View style={styles.legendItem}>
          ...
          <Text style={[styles.legendText, { color: colors.text }]}>Produzido</Text>
        </View>
        <View style={styles.legendItem}>
          <View style={[styles.legendIndicator, { backgroundColor: colors.muted, opacity: 0.6 }]} />
          <Text style={[styles.legendText, { color: colors.text }]}>Meta</Text>
        </View>
      </View>
      ...
      <View>
        <Pressable
          ...
          style={[
            ...
            { backgroundColor: colors.surfaceAlt, borderRadius: radius.md, height: chartHeight },
          ]}
        >
          ...
          <View
            style={[
              styles.tooltip,
              {
                ...
                left: `${(100 / slicedData.length) * (activeIndex ?? 0)}%`,
                marginLeft: `${100 / slicedData.length / 2}%`,
              },
            ]}
            pointerEvents="none"
          >
            <Text style={[styles.tooltipDate, { color: colors.primary }]}>
              Dia {activeData.label.slice(-2)}
            </Text>
            <Text style={[styles.tooltipText, { color: colors.success }]}>
              Prod:{' '}
              <Text style={{ fontWeight: 'bold' }}>{formatNumber(activeData.produced, 0)}</Text>
            </Text>
            <Text style={[styles.tooltipText, { color: colors.muted }]}>
              Meta: <Text style={{ fontWeight: 'bold' }}>{formatNumber(activeData.meta, 0)}</Text>
            </Text>
          </View>
        )}
      </View>
      ...
      <View style={styles.labelsContainer}>
        {slicedData.map(d => (
          <Text key={d.label} style={[styles.axisLabel, { color: colors.muted }]}>
            {d.label.slice(-2)}
          </Text>
        ))}
      </View>

      <Text style={[styles.unitText, { color: colors.muted }]}>
        Valores em {unit.toUpperCase()}
      </Text>
    </View>
  );
}
