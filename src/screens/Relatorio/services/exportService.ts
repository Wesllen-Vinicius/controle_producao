...
const generateCsv = (hasProductFilter: boolean, data: DayTotals[] | ProductTotals[]): string => {
  ...
  if (hasProductFilter) {
    ...
    const rows = (data as DayTotals[]).map(d => {
      ...
      return `${new Date(d.date).toLocaleDateString('pt-BR')};${
        d.abate
      };${fmtCsv(d.produced)};${fmtCsv(d.meta)};${fmtCsv(d.diff)};${fmtCsv(compliance)}`;
    });
...
const generateJson = (hasProductFilter: boolean, data: DayTotals[] | ProductTotals[]): string => {
...
export const handleExport = async (
  ...
  try {
    ...
    if (format === 'csv') {
      ...
      Alert.alert('PDF em breve', 'A exportação para PDF será implementada futuramente.');
      return;
    }
    ...
    Alert.alert(
      ...
      (error instanceof Error ? error.message : undefined) ??
        'Ocorreu um problema ao tentar compartilhar o arquivo.'
    );
  }
};
