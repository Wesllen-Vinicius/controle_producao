...
// CORREÇÃO: 'TextInput' foi movido para esta linha, junto com os outros imports de 'react-native'
...
// A importação que estava aqui foi movida para o topo.
export const ProductForm = React.memo(
  ({ editingProduct, onSubmit, onCancel, isBusy, unitsInUse }: Props) => {
    ...
    const handleSubmit = async () => {
      ...
      try {
        const productData = {
          name: name.trim(),
          unit: unit.trim().toUpperCase() as Unit,
      ...
      <View style={styles.formCard}>
        ...
        <View style={styles.formFieldsContainer}>
          <Input
            ...
            autoCorrect={false}
            leftIcon={
              <MaterialCommunityIcons name="package-variant" size={18} color={colors.muted} />
            }
          />
          ...
          <View>
            <Input
              ...
              value={unit}
              onChangeText={t => setUnit(String(t).toUpperCase().slice(0, 10) as Unit)}
              placeholder="UN, KG, L..."
              autoCapitalize="characters"
              maxLength={10}
              autoCorrect={false}
              leftIcon={
                <MaterialCommunityIcons name="scale-balance" size={18} color={colors.muted} />
              }
            />
          ...
          <View style={styles.actionButtonsRow}>
            <View style={styles.mainButtonContainer}>
              <Button
                ...
                leftIcon={
                  <MaterialCommunityIcons
                    name={editingProduct ? 'content-save' : 'plus'}
                    size={16}
          ...
          <View style={styles.infoBox}>
            <Text style={{ color: colors.muted, fontSize: 12, fontWeight: '500', lineHeight: 16 }}>
              💡 A exclusão de produtos está desabilitada para preservar o histórico. Use a edição
              para modificar.
            </Text>
          </View>
        </View>
