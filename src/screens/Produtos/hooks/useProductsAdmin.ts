...
const validateProduct = (
  ...
  if (trimmedName.length > 50)
    return "O nome do produto não pode ter mais de 50 caracteres.";
  ...
  if (normalizedUnit.length > 10)
    return "A unidade deve ter entre 1 e 10 caracteres.";
  if (isNaN(meta) || meta < 0 || meta > 1000)
    return "Informe uma meta válida entre 0 e 1000.";
...
export const useProductsAdmin = () => {
  ...
  const fetchProducts = useCallback(async () => {
    // Esta guarda previne que a função seja executada novamente se já estiver carregando
    if (loading) return;
    setLoading(true);
    try {
    ...
    // CORREÇÃO: Removido [loading]. Agora a função é estável e não será recriada quando o loading mudar.
  }, []);
  ...
  const onRefresh = useCallback(async () => {
    ...
    await fetchProducts();
    setRefreshing(false);
    // A dependência aqui está correta, pois fetchProducts agora é estável.
  }, [fetchProducts]);
  ...
  const saveProduct = useCallback(
    async (productData: Omit<Product, "id">) => {
      ...
      try {
        ...
        Alert.alert(
          "Erro ao Salvar",
          e.message ?? "Não foi possível adicionar o produto."
        );
        return { success: false };
      } finally {
        setLoading(false);
      }
      // A dependência aqui está correta.
    },
    [list, fetchProducts]
  );
  ...
  const updateProduct = useCallback(
    async (id: string, productData: Omit<Product, "id">) => {
      ...
      try {
        ...
        Alert.alert(
          "Erro ao Atualizar",
          e.message ?? "Não foi possível atualizar o produto."
        );
        return { success: false };
      } finally {
        setLoading(false);
